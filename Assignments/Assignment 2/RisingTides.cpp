...
void check(Grid<bool>& ans,
           Queue<GridLocation>& q,
           const Grid<double>& terrain,
           int r, int c, double height) {
    GridLocation l(r, c);
    if (ans.inBounds(r, c) && ans[r][c] == false && height >= terrain[r][c]) {
        ans[r][c] = true;
        q.enqueue(l);
    }
}
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> ans(terrain.numRows(), terrain.numCols(), false);
    Queue<GridLocation> q;
    for (GridLocation gl : sources) {
        if (height >= terrain[gl.row][gl.col]) {
            ans[gl.row][gl.col] = true;
            q.enqueue(gl);
        }
    }
    ...
    while (!q.isEmpty()) {
        ...
        int r = loc.row;
        int c = loc.col;
        check(ans, q, terrain, r + 1, c, height);
        check(ans, q, terrain, r - 1, c, height);
        check(ans, q, terrain, r, c + 1, height);
        check(ans, q, terrain, r, c - 1, height);
    }
    return ans;
}
