...
int hours(const Set<Shift>& shifts) {
    int sum = 0;
    for (Shift s : shifts) {
        sum += lengthOf(s);
    }
...
bool checkSkip(const Set<Shift>& now, Shift temp, int maxHours) {
    int nowHours = hours(now) + lengthOf(temp);
    if (nowHours > maxHours) {
        return true;
    }
    for (Shift s : now) {
        if (overlapsWith(s, temp)) {
            return true;
        }
    }
    return false;
...
Set<Shift> highestValueScheduleForHelper(const Set<Shift>& copy, const Set<Shift>& now,
                                         int maxHours) {
    // base case
    if (copy.isEmpty()) {
        return now;
    ...
    Shift temp = copy.first();
    Set<Shift> remain = copy - temp;

    if (checkSkip(now, temp, maxHours)) {
        return highestValueScheduleForHelper(remain, now, maxHours);
    } else {

        Set<Shift> ifChoose = now + temp;
        Set<Shift> res1 = highestValueScheduleForHelper(remain, now, maxHours);
        Set<Shift> res2 = highestValueScheduleForHelper(remain, ifChoose, maxHours);
        if (value(res1) > value(res2)) {
            return res1;
        } else {
            return res2;
        }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) {
        error("Max hours must be positive!");
    }
    if (maxHours == 0) {
        return {};
    }
    ...
    Set<Shift> now = {};
    return highestValueScheduleForHelper(copy, now, maxHours);
}
