...
void drawSierpinskiTriangle(GWindow& window,
    ...
    if (order == 0) {
        ...
        double xa = (x0 + x1) / 2, ya = (y0 + y1) / 2;
        double xb = (x0 + x2) / 2, yb = (y0 + y2) / 2;
        double xc = (x1 + x2) / 2, yc = (y1 + y2) / 2;
        drawSierpinskiTriangle(window, x0, y0, xa, ya, xb, yb, order - 1);
        drawSierpinskiTriangle(window, x1, y1, xa, ya, xc, yc, order - 1);
        drawSierpinskiTriangle(window, x2, y2, xc, yc, xb, yb, order - 1);
    }
}
