...
Set<string> allEmphasesOfHelper(Vector<string>& tokens, Vector<string> now, int p) {
    Set<string> res;
    if (p == tokens.size()) {
        string ans;
        for (string token : now) {
            ans += token;
        }
        res.add(ans);
        return res;
    ...
    if (!isalpha(tokens[p][0])) {
        res += allEmphasesOfHelper(tokens, now, p + 1);
        return res;
    }
    // 是单词
...
Set<string> allEmphasesOf(const string& sentence) {
    string lower = toLowerCase(sentence);
...
STUDENT_TEST("Test for only one word.") {
    Set<string> expected = {
        "hi",
        "HI",
    };

    EXPECT_EQUAL(allEmphasesOf("Hi"), expected);
}

STUDENT_TEST("Test for non-alpha input.") {
    Set<string> expected = {
        "123",
    };

    EXPECT_EQUAL(allEmphasesOf("123"), expected);
}
