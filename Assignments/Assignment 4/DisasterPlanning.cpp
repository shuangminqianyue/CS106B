...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork,
    ...
    if (remained.isEmpty() && numCities >= 0) {
        return true;
    }
    // case1: 选择该城市
    // 1.choose
    string city = remained.first();
    Set<string> neighbors = roadNetwork[city];
    Set<string> left = remained - city - neighbors;
    supplyLocations += city;
    // 2.explore
    if (canBeMadeDisasterReadyHelper(roadNetwork, numCities - 1, supplyLocations, left)) {
        return true;
    }
    // 3.unchoose
    supplyLocations -= city;
    ...
    for (string neighbor : neighbors) {
        ...
        Set<string> left = remained - neighbor - neighborsOfNeighbor;
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            ...
                            Set<string>& supplyLocations) {
    if (numCities < 0) {
        error("Numbers of cities must be positive.");
    }
    Set<string> remained;
    for (string city : roadNetwork) {
        remained += city;
    }
    return canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, remained);
}
