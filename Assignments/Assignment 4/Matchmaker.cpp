...
bool matchingContains(Set<Pair>& matching, string p) {
    for (Pair m : matching) {
        if (m.first() == p || m.second() == p) {
            return true;
        }
    }
    return false;
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks,
    ...
    for (string p2 : possibleLinks[p1]) {
        if (matchingContains(matching, p2)) {
            continue;
        }
        Map<string, Set<string>> remain = possibleLinks;
        remain.remove(p1);
        remain.remove(p2);
        Set<Pair> ifPlus = matching + Pair(p1, p2);
        if (hasPerfectMatching(remain, ifPlus)) {
            matching += ifPlus;
            return true;
        }
    }
...
/* 1.基本思路：每次从未匹配的节点中选择一个节点，尝试与所有可能的配对节点进行匹配，然后递归地处理剩下的节点。
 * 2.递归终止条件：当所有节点都被考虑过时（即没有未匹配的节点），返回当前匹配的总权值。
 * 3.状态转移：对于每一个匹配的选择，递归计算剩余节点的最大权匹配，返回最优结果。
 */
Set<Pair> maximumWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks,
                                      int& currentWeight) {
    Set<Pair> maxPair;
    // 递归结束
    if (possibleLinks.isEmpty()) {
    ...
    Map<string, Map<string, int>> remain1 = possibleLinks;
    remain1.remove(node);
    // 不选，直接递归剩余的
    int unChoose = currentWeight;
    Set<Pair> pairUnChoose = maximumWeightMatchingHelper(remain1, unChoose);
    // 选
    int choose = currentWeight;
    Set<Pair> pairChoose;
    for (string second : possibleLinks[node]) {
        if (!possibleLinks.containsKey(second)) {
            continue;
        }
        Map<string, Map<string, int>> remain2 = possibleLinks;
        remain2.remove(node);
        remain2.remove(second);
        Pair pair(node, second);
        int chooseSecond = currentWeight + possibleLinks[node][second];
        Set<Pair> pairChooseSecond = maximumWeightMatchingHelper(remain2, chooseSecond) + pair;
        if (chooseSecond > choose) {
            choose = chooseSecond;
    ...
    if (choose > unChoose) {
        maxPair = pairChoose;
        currentWeight = choose;
    }
    else {
        maxPair = pairUnChoose;
        currentWeight = unChoose;
    }
    return maxPair;
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    int currentWeight = 0;
    return maximumWeightMatchingHelper(possibleLinks, currentWeight);
}
...
STUDENT_TEST("hasPerfectMatching works on a world that cannot have perfect match.") {
    /* Here's the world:
     *      A
     *      |
     *      B
