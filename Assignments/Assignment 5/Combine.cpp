...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<Vector<DataPoint>> group1 = sequences.subList(0, length / 2);
    Vector<Vector<DataPoint>> group2 = sequences.subList(length / 2);
    Vector<DataPoint> group1Vector = combine(group1);
    Vector<DataPoint> group2Vector = combine(group2);
    // merge
    Vector<DataPoint> res = merge(group1Vector, group2Vector);
    return res;
}
...
STUDENT_TEST("Merges two lists of same size.") {
    Vector<DataPoint> one = { { "B" , 3 }};
    Vector<DataPoint> two = { { "A" , 2 }};
