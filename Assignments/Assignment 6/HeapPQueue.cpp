...
void HeapPQueue::resize(DataPoint* &elems) {
    DataPoint* newArray = new DataPoint[allocatedSize * 2];
...
void HeapPQueue::bubbleUp(DataPoint* elems) {
    ...
    int father = logicalSize / 2;
    while (elems[curr].weight <= elems[father].weight && father >= 1) {
        swap(elems[curr], elems[father]);
        curr = father;
        father = curr / 2;
    }
}
...
void HeapPQueue::bubbleDown(DataPoint* elems) {
    ...
    while (curr <= logicalSize / 2) {
        ...
        if (elems[curr].weight < elems[lower].weight) {
            break;
        }
        swap(elems[curr], elems[lower]);
        curr = lower;
    }
}
...
DataPoint HeapPQueue::peek() const {
    /* TODO: Delete the next line and implement this. */
    if (isEmpty()) {
        error("can not peek on empty PQueue.");
    }
    return elems[1];
}
